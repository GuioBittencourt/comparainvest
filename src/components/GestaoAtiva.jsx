...
export default function GestaoAtiva({ user, targetUser = null, adminMode = false, readOnly = false }) {
  const effectiveUser = targetUser || user;
  const userId = effectiveUser?.id || null;
  const isAdminViewing = !!(adminMode && targetUser);
  const canEditAdminTarget = !!(user?.is_admin && targetUser?.is_aluno);
  const readOnlyMode = !!(readOnly || (isAdminViewing && !canEditAdminTarget));
  const canEdit = !readOnlyMode;
  const isAluno = !!(effectiveUser?.is_aluno && !user?.is_admin);
  const canManageCats = canEdit && !isAluno;
  ...
  useEffect(() => { if (!readOnlyMode) saveData(data, userId); }, [data, userId, readOnlyMode]);
  ...
  if (!profile || editProfile) {
    if (readOnlyMode) {
      return (<div><BannerFinanceiro /><div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, color: C.textDim, fontSize: 13, lineHeight: 1.7 }}>Este usuário ainda não configurou a Gestão Ativa. Como não é aluno, o acesso do ADM fica apenas em modo observação.</div></div>);
    }
  ...
  const addCat = (preset, limit) => { if (!canEdit) return; if (!isPremium && categories.length >= FREE_MAX) { setShowUpgrade(true); return; } if (categories.find((c) => c.id === preset.id)) return; setData((p) => ({ ...p, categories: [...(p.categories || []), { ...preset, limit: parseFloat(limit) || 0 }] })); setShowAddCat(false); setSelPreset(null); setCatLimit(""); setCustomName(""); setCustomLimit(""); };
  const addExp = () => { if (!canEdit || !selectedCat || !expVal) return; setData((p) => ({ ...p, expenses: [...(p.expenses || []), { id: Date.now(), categoryId: selectedCat, value: parseFloat(expVal) || 0, desc: expDesc.trim(), date: new Date().toISOString() }] })); setExpVal(""); setExpDesc(""); setShowAddExp(false); setSelectedCat(null); };
    ...
    <div>
      {isAdminViewing && (
        <div style={{ marginBottom: 14, padding: "10px 12px", borderRadius: 12, border: `1px solid ${canEdit ? C.accentBorder : C.border}`, background: canEdit ? `${C.accent}0D` : "rgba(255,255,255,0.030)", color: C.textDim, fontSize: 12, lineHeight: 1.55 }}>
          Visualizando Gestão Ativa de <strong style={{ color: C.white }}>{targetUser?.nome} {targetUser?.sobrenome || ""}</strong>. {canEdit ? "Modo edição liberado por ser aluno." : "Somente leitura: usuário não marcado como aluno."}
        </div>
      )}
      <BannerFinanceiro />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        ...
        <button onClick={() => setEditProfile(true)} style={{ padding: "5px 12px", borderRadius: 6, fontSize: 9, fontFamily: MN, cursor: "pointer", background: `${C.accent}15`, color: C.accent, border: `1px solid ${C.accent}30` }}>Perfil de Gestão →</button>
      </div>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 14, marginBottom: 12 }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 8, alignItems: "start" }}>
          <div style={{ minWidth: 0, textAlign: "center" }}>
            ...
            <div style={{ fontSize: 12, color: C.white, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{profile.profissao}</div>
          </div>
          <div style={{ textAlign: "center", minWidth: 0 }}>
            <div style={{ fontSize: 9, color: C.textMuted, marginBottom: 4 }}>Renda</div>
            <div style={{ fontFamily: MN, fontSize: isMobile ? 12 : 14, fontWeight: 800, color: C.accent, whiteSpace: "nowrap" }}><span style={{ fontSize: isMobile ? 9 : 10 }}>R$</span><span>{numFmt(renda, 0)}</span></div>
          </div>
          <div style={{ textAlign: "center", minWidth: 0 }}>
            <div style={{ fontSize: 9, color: C.textMuted, marginBottom: 4 }}>Fixas ({numFmt(fixasPct, 0)}%)</div>
            <div style={{ fontFamily: MN, fontSize: isMobile ? 12 : 14, fontWeight: 700, color: fixasPct > 60 ? C.red : C.blue, whiteSpace: "nowrap" }}><span style={{ fontSize: isMobile ? 9 : 10 }}>R$</span><span>{numFmt(fixas, 0)}</span></div>
          </div>
          <div style={{ textAlign: "center", minWidth: 0 }}>
            <div style={{ fontSize: 9, color: C.textMuted, marginBottom: 4 }}>Disponível</div>
            <div style={{ fontFamily: MN, fontSize: isMobile ? 12 : 14, fontWeight: 800, color: C.accent, whiteSpace: "nowrap" }}><span style={{ fontSize: isMobile ? 9 : 10 }}>R$</span><span>{numFmt(variavel, 0)}</span></div>
          </div>
        </div>
      </div>
      ...
      {categories.length > 0 && (
        <div style={{ background: totalRem >= 0 ? "linear-gradient(135deg,#0D3320,#0D1117)" : "linear-gradient(135deg,#3d0d0d,#0D1117)", border: `1px solid ${totalRem >= 0 ? C.accentBorder : C.red+"40"}`, borderRadius: 12, padding: 18, marginBottom: 12, textAlign: "center" }}>
          <div style={{ fontSize: 9, color: C.textMuted, fontFamily: MN }}>SALDO VARIÁVEIS</div>
          <div style={{ ...moneyCompactStyle, fontSize: "clamp(24px, 7vw, 32px)", color: totalRem >= 0 ? C.accent : C.red }}>R$ {numFmt(Math.abs(totalRem), 2)}</div>
          {totalRem < 0 && <div style={{ fontSize: 10, color: C.red }}>Orçamento estourado!</div>}
          <div style={{ height: 4, background: C.border, borderRadius: 3, marginTop: 8, overflow: "hidden" }}><div style={{ width: `${Math.min(100, totalLimit > 0 ? (totalSpent/totalLimit)*100 : 0)}%`, height: "100%", borderRadius: 3, background: totalSpent/totalLimit > 0.9 ? C.red : totalSpent/totalLimit > 0.7 ? C.yellow : C.accent }} /></div>
          <div style={{ display: "flex", justifyContent: "center", gap: 16, marginTop: 8 }}><span style={{ fontSize: 10, color: C.textMuted }}>Orçamento: R$ {numFmt(totalLimit, 0)}</span><span style={{ fontSize: 10, color: C.red }}>Gasto: R$ {numFmt(totalSpent, 0)}</span></div>
        </div>
      )}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        <span style={{ fontFamily: MN, fontSize: 10, color: C.textDim }}>CATEGORIAS ({categories.length})</span>
        <div style={{ display: "flex", gap: 6 }}>
          {categories.length > 0 && canManageCats && <button onClick={() => { if (confirm("Tem certeza que deseja resetar todos os gastos deste mês? As categorias serão mantidas, mas os lançamentos serão apagados.")) { setData((p) => ({ ...p, expenses: [] })); } }} style={{ padding: "3px 8px", borderRadius: 5, fontSize: 9, fontFamily: MN, cursor: "pointer", background: `${C.red}15`, color: C.red, border: `1px solid ${C.red}30` }}>Resetar</button>}
          {canManageCats && <button onClick={() => { if (!isPremium && categories.length >= FREE_MAX) { setShowUpgrade(true); return; } setShowAddCat(true); }} style={{ padding: "3px 8px", borderRadius: 5, fontSize: 9, fontFamily: MN, cursor: "pointer", background: `${C.accent}15`, color: C.accent, border: `1px solid ${C.accent}30` }}>+ Categoria</button>}
        </div>
      </div>
