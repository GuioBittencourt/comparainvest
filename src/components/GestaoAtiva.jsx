...
const PRESETS = [
  { id: "mercado", label: "Mercado / Alimentação", icon: <IconMercado size={18} />, color: C.accent },
  { id: "transporte", label: "Transporte / Gasolina", icon: <IconTransporte size={18} />, color: C.textDim },
  { id: "diversao", label: "Diversão / Lazer", icon: <IconDiversao size={18} />, color: C.textDim },
  { id: "saude", label: "Saúde / Farmácia", icon: <IconSaude size={18} />, color: C.textDim },
  { id: "pets", label: "Pets", icon: <IconPets size={18} />, color: C.yellow },
  { id: "cursos", label: "Cursos e Livros", icon: <IconCursos size={18} />, color: "#818CF8" },
  { id: "outros", label: "Outros", icon: <IconOutros size={18} />, color: C.textDim },
];

const HEALTHY = [
  { label: "Contas Fixas", pct: 60, color: C.textDim, desc: "Aluguel, internet, água, luz, condomínio, plano celular..." },
  { label: "Diversão", pct: 10, color: C.textDim, desc: "Lazer, restaurantes, cinema, viagens curtas..." },
  { label: "Invest. Longo Prazo", pct: 10, color: C.accent, desc: "Ações, FIIs, Tesouro IPCA+, previdência..." },
  { label: "Invest. Curto Prazo", pct: 15, color: C.textDim, desc: "Reserva emergência, Tesouro Selic, CDB liquidez..." },
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto" }}>
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        ...
        <h2 style={{ fontFamily: TN, fontSize: 20, fontWeight: 400, color: C.white, margin: "0 0 6px" }}>Perfil de Gestão</h2>
...
export default function GestaoAtiva({ user }) {
  ...
  const addCat = (preset, limit) => { if (!isPremium && categories.length >= FREE_MAX) { alert("Plano atual: limite de 4 categorias. Faça upgrade para liberar categorias ilimitadas."); return; } if (categories.find((c) => c.id === preset.id)) return; setData((p) => ({ ...p, categories: [...(p.categories || []), { ...preset, limit: parseFloat(limit) || 0 }] })); setShowAddCat(false); setSelPreset(null); setCatLimit(""); setCustomName(""); setCustomLimit(""); };
  ...
  const distData = [{ name: "Fixas", value: fixas, fill: "#6EE7B7" }, { name: "Variáveis", value: totalSpent, fill: "#94A3B8" }, { name: "Disponível", value: Math.max(0, renda - fixas - totalSpent), fill: C.accent }].filter((d) => d.value > 0);
