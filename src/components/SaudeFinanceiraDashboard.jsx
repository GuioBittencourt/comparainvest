...
function Dividas({ data }) {
  ...
  const r = diagnostico.base;
  const lista = [
    ...(data.outrasContas || []).map((d) => ({ nome: d.nome || d.tipo || "Dívida", parcela: d.parcela, total: d.valorTotal, tipo: "Conta quitável" })),
    ...(data.consignados || []).map((d) => ({ nome: d.nome || "Consignado", parcela: d.parcela, total: d.valorQuitacao, tipo: d.descontaFolha ? "Consignado em folha" : "Consignado fora da folha" })),
    ...(data.agiotas || []).map((d) => ({ nome: d.nome || "Agiota", parcela: r.agiotas.jurosMensais, total: d.valorQuitacao, tipo: "Atenção máxima" })),
    ...(data.contasAtrasadas || []).map((d) => ({ nome: d.nome || "Conta atrasada", parcela: d.valorConta, total: d.valorTotal, tipo: "Atrasada" })),
  ];
  const ordenada = lista.sort((a, b) => (Number(b.parcela || 0) - Number(a.parcela || 0)) || (Number(a.total || 0) - Number(b.total || 0)));
  return <Card>
    <h3 style={{ color: C.white, margin: 0, fontSize: 22 }}>Dívidas e pressão mensal</h3>
    <p style={{ color: C.textDim, fontSize: 13, lineHeight: 1.7 }}>A ordem abaixo prioriza liberação de fluxo. Depois vamos permitir ajustes por prioridade pessoal/emocional.</p>
    {ordenada.length === 0 ? <p style={{ color: C.textDim }}>Nenhuma dívida quitável registrada.</p> : ordenada.map((d, idx) => <div key={`${d.nome}-${idx}`} style={{ display: "grid", gridTemplateColumns: "auto 1fr auto", gap: 12, alignItems: "center", borderBottom: `1px solid ${C.border}`, padding: "12px 0" }}>
      ...
      <div style={{ textAlign: "right", fontFamily: MN, fontSize: 12 }}><div style={{ color: C.accent }}>{formatarBRL(d.parcela)}</div><div style={{ color: C.textDim }}>{formatarBRL(d.total)}</div></div>
    </div>)}
  </Card>;
}

function Independencia({ data }) {
  const r = gerarDiagnosticoSaude(data).base;
  const aporteMensal = Math.max(0, r.saldoMes);
  const aporteInicial = r.investimentos;
  return <Card>
    <h3 style={{ color: C.white, margin: 0, fontSize: 22 }}>Independência financeira</h3>
    <p style={{ color: C.textDim, fontSize: 13, lineHeight: 1.7 }}>Prévia automática. O simulador completo entra depois com taxa configurável e projeção de 1 a 35 anos.</p>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10 }}>
      <Metric label="Aporte inicial" value={formatarBRL(aporteInicial)} color={C.accent} />
      <Metric label="Aporte possível" value={formatarBRL(aporteMensal)} color={aporteMensal > 0 ? C.accent : C.red} />
