...
import ViradasMes from "./ViradasMes";
...
export default function SaudeFinanceiraDashboard({ data, setData, onEdit, user, initialTab = "financeiro", readOnly = false, targetUser = null, linhasExtras = [] }) {
  const [aba, setAba] = useState(initialTab || "financeiro");
  useEffect(() => { setAba(initialTab || "financeiro"); }, [initialTab]);
  const [showGate, setShowGate] = useState(false);
  const [showVirada, setShowVirada] = useState(false);
    ...
    <div style={{ display: "grid", gap: 16, width: "100%", boxSizing: "border-box", minWidth: 0 }}>
      ...
      {showVirada && isAdmin && (
        <ViradasMes
          ...
          ajustes={{}}
          onClose={() => setShowVirada(false)}
        />
      )}
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 10, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        {ABAS.map((a) => (
        ...
        {isAdmin && targetUser && (
          <button onClick={() => setShowVirada(true)}
            style={{ marginLeft: "auto", padding: "8px 14px", borderRadius: 999, cursor: "pointer", fontSize: 12, fontFamily: MN, fontWeight: 700,
              background: "linear-gradient(135deg, #25D166, #1aaa54)",
