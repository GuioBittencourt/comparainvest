  ...
  return (
    <div style={{ padding: "24px 28px", maxWidth: 700, margin: "0 auto" }}>
      ...
      <button onClick={() => setSubTab("gestao")} style={{ width: "100%", textAlign: "left", cursor: "pointer", background: "linear-gradient(135deg, #0D3320 0%, #0D1117 100%)", border: `1px solid ${C.accentBorder}`, borderRadius: 16, padding: "20px 22px", marginBottom: 16, display: "flex", alignItems: "center", gap: 16 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontFamily: MN, fontSize: 14, fontWeight: 700, color: C.accent }}>Gestão Ativa</span><span style={{ fontSize: 9, padding: "2px 8px", borderRadius: 4, background: `${C.accent}15`, color: C.accent, fontFamily: MN }}>Disponível</span></div>
      ...
      {[
        ...
        { icon: "📊", title: "Análise de Investimentos", desc: "Indicadores fundamentalistas, balanços, setores.", level: "Intermediário", color: C.blue, tag: "Em breve" },
        { icon: "🧠", title: "Estratégias Avançadas", desc: "Cenários macro, marcação a mercado, hedge.", level: "Avançado", color: C.purple, tag: "Em breve" },
      ].map((c) => (
        <div key={c.title} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: "20px 22px", marginBottom: 12, display: "flex", alignItems: "center", gap: 16, opacity: 0.6 }}>
          ...
          <div style={{ flex: 1 }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontFamily: MN, fontSize: 14, fontWeight: 700, color: C.white }}>{c.title}</span><span style={{ fontSize: 9, padding: "2px 8px", borderRadius: 4, background: `${c.color}15`, color: c.color, fontFamily: MN }}>{c.level}</span><span style={{ fontSize: 9, padding: "2px 8px", borderRadius: 4, background: `${C.yellow}15`, color: C.yellow, fontFamily: MN }}>{c.tag}</span></div><p style={{ fontSize: 12, color: C.textDim, margin: "4px 0 0", lineHeight: 1.6 }}>{c.desc}</p></div>
        </div>
      ))}
