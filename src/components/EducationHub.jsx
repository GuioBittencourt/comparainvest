...
import AulasHub from "./AulasHub";
import AulaPlayer from "./AulaPlayer";

export default function EducationHub({ onBack, onTrack, user, targetUser = null, adminMode = false, readOnly = false }) {
  ...
  if (subTab === "gestao") {
    return (
      <div style={{ padding: "24px 28px", maxWidth: 700, margin: "0 auto" }}>
        ...
        <GestaoAtiva user={user} targetUser={targetUser} adminMode={adminMode} readOnly={readOnly} />
      </div>
    );
  }

  if (subTab === "aulas" && !aulaAtiva) {
    return (
      <div style={{ padding: "24px 28px" }}>
        <AulasHub onAula={(id) => setAulaAtiva(id)} onBack={() => setSubTab("hub")} />
      </div>
    );
  }
  ...
  if (subTab === "aulas" && aulaAtiva) {
    return (
      <div style={{ padding: "24px 28px" }}>
        <AulaPlayer
          aulaId={aulaAtiva}
          onBack={() => setAulaAtiva(null)}
          onAula={(id) => setAulaAtiva(id)}
          user={user}
        />
      </div>
    );
    ...
    <div style={{ padding: "24px 28px", maxWidth: 700, margin: "0 auto" }}>
      ...
      <button
        ...
        style={{
          ...
          background: "linear-gradient(135deg, rgba(0,229,160,0.12) 0%, rgba(13,17,23,0.98) 100%)",
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 9, padding: "2px 8px", borderRadius: 4, background: `${C.accent}15`, color: C.accent, fontFamily: MN }}>5 vídeos</span>
          </div>
          <p style={{ fontSize: 12, color: C.textDim, margin: "4px 0 0", lineHeight: 1.6 }}>Curso completo de educação financeira e investimentos com Guilherme Bittencourt.</p>
        </div>
      ...
      <button
        ...
        onMouseEnter={(e) => { e.currentTarget.style.borderColor = C.blue || "#3B82F6"; e.currentTarget.style.transform = "translateY(-1px)"; }}
        onMouseLeave={(e) => { e.currentTarget.style.borderColor = `${C.blue || "#3B82F6"}55`; e.currentTarget.style.transform = "translateY(0)"; }}
      >
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
      ...
      <button onClick={() => setSubTab("gestao")} style={{ width: "100%", textAlign: "left", cursor: "pointer", background: "linear-gradient(135deg, #0D3320 0%, #0D1117 100%)", border: `1px solid ${C.accentBorder}`, borderRadius: 16, padding: "20px 22px", marginBottom: 16, display: "flex", alignItems: "center", gap: 16 }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontFamily: TN, fontSize: 16, fontWeight: 400, color: C.accent }}>Gestão Ativa</span>
            <span style={{ fontSize: 9, padding: "2px 8px", borderRadius: 4, background: `${C.accent}15`, color: C.accent, fontFamily: MN }}>Disponível</span>
          </div>
      ...
      {/* Em breve */}
      {[
        { title: "Conceitos Básicos", desc: "Renda fixa vs variável, juros compostos, inflação, Selic.", level: "Iniciante", color: C.green, tag: "Em breve" },
        { title: "Análise de Investimentos", desc: "Indicadores fundamentalistas, balanços, setores.", level: "Intermediário", color: C.textDim, tag: "Em breve" },
        { title: "Estratégias Avançadas", desc: "Cenários macro, marcação a mercado, hedge.", level: "Avançado", color: C.textDim, tag: "Em breve" },
      ].map((c) => (
        <div key={c.title} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: "20px 22px", marginBottom: 12, display: "flex", alignItems: "center", gap: 16, opacity: 0.6 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontFamily: FN, fontSize: 14, fontWeight: 600, color: C.white }}>{c.title}</span>
