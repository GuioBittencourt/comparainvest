...
export default function ExtratoFuturo({ data }) {
  ...
  // null = panorama completo, string = mes filtrado
  ...
  const linhasFiltradas = filtroMes ? linhas.filter((l) => l.mes === filtroMes) : linhas;
  const mesSelecionado = filtroMes ? linhas.find((l) => l.mes === filtroMes) : null;
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 18, padding: 18, boxSizing: "border-box", width: "100%" }}>
      ...
      {/* Tabela (panorama completo ou mês filtrado) */}
      <div style={{ overflowX: "auto", marginTop: 12 }}>
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: filtroMes ? 320 : 1180, fontFamily: FN }}>
          <thead>
            <tr>
