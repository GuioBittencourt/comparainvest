...
const ROWS = [
  ...
  { key: "_ajuste", label: "Ajustar investimento" },
  { key: "_acao", label: "Ação estratégica" },
];
...
// Card compacto mês filtrado — mesma ordem lógica da tabela
function MesFiltradoCard({ linha, ajustes, setInvestimentoManual }) {
  ...
  return (
    <div style={{ marginTop: 12 }}>
      ...
      {/* Grid 2 colunas — mesma ordem da tabela, linha por linha */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        {CARD_ROWS.map((row) => (
          <div key={row.key} style={{
            background: C.cardAlt,
            border: `1px solid ${isSaldo(row.key) ? (Number(linha[row.key]) < 0 ? C.red+"40" : C.accentBorder+"80") : C.border}`,
...
export default function ExtratoFuturo({ data }) {
  ...
  return (
    // overflow: hidden impede a tabela de empurrar o layout pra direita
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 18, padding: 18, boxSizing: "border-box", width: "100%", overflow: "hidden" }}>
      ...
      {/* Tabela panorama OU card mês filtrado */}
      {linhaFiltrada ? (
        <MesFiltradoCard
          linha={linhaFiltrada}
          ajustes={ajustes}
        ...
        <div style={{ overflowX: "auto", marginTop: 12 }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 1180, fontFamily: FN }}>
            ...
            <tbody>
              {ROWS.filter(r => !r.key.startsWith("_")).map((row) => (
                <tr key={row.key}>
