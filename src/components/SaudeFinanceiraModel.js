...
export function carregarSaudeFinanceira() {
  ...
  try {
    ...
    return { ...novoModeloSaude(), ...JSON.parse(raw) };
  } catch {
    return novoModeloSaude();
  }
}

export function salvarSaudeFinanceira(data) {
  if (typeof window === "undefined") return;
  try {
    localStorage.setItem(
      SAUDE_STORAGE_KEY,
      JSON.stringify({ ...data, atualizadoEm: new Date().toISOString() })
    );
  } catch {}
}
...
export function calcularResumoSaude(data) {
  ...
  const totalEntradas =
    numero(entradas.salario1) +
    numero(entradas.salario2) +
    numero(entradas.adiantamento1) +
    numero(entradas.adiantamento2) +
    numero(entradas.bonus) +
    vaVrDinheiro +
    outrasFontes;

  const outrosFixos = (fixas.outros || [])
    .filter((i) => !i.quitavel)
    .reduce((s, i) => s + numero(i.valor), 0);

  const totalFixas =
    numero(fixas.moradia) +
    numero(fixas.condominio) +
    numero(fixas.agua) +
    numero(fixas.luz) +
    numero(fixas.gas) +
    numero(fixas.mercado) +
    numero(fixas.transporte) +
    numero(fixas.celular) +
    numero(fixas.internet) +
    (fixas.convenio?.descontaFolha ? 0 : numero(fixas.convenio)) +
    numero(fixas.uber) +
    (fixas.streaming?.noCartao ? 0 : numero(fixas.streaming)) +
    numero(fixas.pet) +
    numero(fixas.farmacia) +
    numero(fixas.cabelo) +
    (fixas.academia?.noCartao ? 0 : numero(fixas.academia)) +
    numero(fixas.pensaoPaga) +
    numero(fixas.educacao) +
    (fixas.seguro?.noCartao ? 0 : numero(fixas.seguro)) +
    outrosFixos;

  const totalCartoes = (data.cartoes || []).reduce((s, c) => s + numero(c.faturaAtual), 0);
  const totalOutrasContas = (data.outrasContas || []).reduce((s, d) => s + numero(d.parcela), 0);
  const totalConsignados = (data.consignados || []).filter((c) => !c.descontaFolha).reduce((s, c) => s + numero(c.parcela), 0);
  const totalAgiotas = (data.agiotas || []).reduce((s, a) => {
    if (a.juros?.tipo === "percentual") return s + (numero(a.valorQuitacao) * numero(a.juros.valor)) / 100;
    return s + numero(a.juros?.valor);
  }, 0);
  ...
  return {
    ...
    saldoAtual,
    patrimonioInvestido,
    essencialPct: totalEntradas > 0 ? (totalFixas / totalEntradas) * 100 : 0,
  };
}
...
export function cruzarRecorrentesComCartoes(data) {
  ...
  const fixas = data.contasFixas || {};
  const candidatos = [
    ["streaming", fixas.streaming],
    ["academia", fixas.academia],
    ["seguro", fixas.seguro],
    ...(fixas.outros || []).map((o) => [o.nome || "Outro", o]),
  ];
  candidatos.forEach(([nome, item]) => {
    if (!item?.noCartao || !item?.cartaoId) return;
    const idx = cartoes.findIndex((c) => c.id === item.cartaoId);
    if (idx >= 0) cartoes[idx].recorrentes.push({ id: novoId("rec"), nome, valor: numero(item.valor) });
  });
  return { ...data, cartoes };
}
