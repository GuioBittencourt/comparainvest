...
export default function SaudeFinanceiraResumo({ data, setData, onBack }) {
  ...
  const concluir = () => {
    salvarSnapshotSaude(data);
    setData((p) => ({ ...p, questionarioCompleto: true, stepAtual: "resumo" }));
    setSalvo(true);
  };
  ...
  return (
    <Card>
      ...
      {salvo && <div style={{ border: `1px solid ${C.accentBorder}`, background: `${C.accent}10`, color: C.accent, borderRadius: 12, padding: 10, fontSize: 12, marginTop: 12 }}>Diagnóstico salvo. Agora você pode acessar as abas de Financeiro, Distribuição, Extrato Futuro, Dívidas, Independência e Relatório.</div>}
