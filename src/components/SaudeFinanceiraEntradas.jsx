...
export default function SaudeFinanceiraEntradas({ data, setData, onNext }) {
  ...
  const temSegundaRenda = !!entradas.temSalario2 || (Number(entradas.salario2?.valor) || 0) > 0;
  ...
  return <Card><TituloBloco etapa="Bloco 1 de 7" titulo="Entradas" subtitulo="Vamos começar pelo que realmente entra na sua conta. O salário precisa ser líquido: o valor depois dos descontos." />
    ...
    <OpcaoSimNao label="Tem segunda renda fixa? (cônjuge, segundo emprego ou renda familiar recorrente)" value={temSegundaRenda} onChange={(v) => updateEntradas({ temSalario2: v, salario2: { ...entradas.salario2, valor: v ? entradas.salario2.valor || 0 : 0, confirmadoLiquido: false } })} />
    {temSegundaRenda && <><CampoMoeda label="Valor líquido da segunda renda" value={entradas.salario2.valor} onChange={(v) => updateEntradas({ salario2: { ...entradas.salario2, valor: v } })} /><OpcaoSimNao label="Essa segunda renda também é líquida?" value={entradas.salario2.confirmadoLiquido} onChange={(v) => updateEntradas({ salario2: { ...entradas.salario2, confirmadoLiquido: v } })} /></>}
