...
export default function MeuNegocio({ user }) {
  ...
  const alerts = useMemo(() => {
    if (totalR === 0 || !active) return [];
    const list = [];
    if (custoPct > bench.custoTotal + 10) list.push({ type: "danger", area: "Custos Totais", detail: `Seus custos representam ${fmtPct(custoPct)} do faturamento. O saudável para ${seg?.label} é até ${bench.custoTotal}%. Potencial de economia: ${fmtBRL((custoPct - bench.custoTotal) / 100 * totalR)}/mês.` });
    if (margem < bench.margemLiquida - 5) list.push({ type: "warning", area: "Margem Líquida", detail: `Sua margem é ${fmtPct(margem)}, abaixo da meta de ${bench.margemLiquida}%. Revise as categorias com maior peso.` });
    despPorCat.forEach(([cat, val]) => { const p = (val / totalR) * 100; if (p > 30) list.push({ type: "warning", area: cat, detail: `"${cat}" consome ${fmtPct(p)} do faturamento (${fmtBRL(val)}/mês). Avalie se gera retorno proporcional.` }); });
    return list;
  }, [totalR, totalD, custoPct, margem, bench, seg, despPorCat, active]);
    ...
    return (
      <div>
        <div style={{ marginBottom: 20 }}>
          ...
          <h2 style={{ fontFamily: TN, fontSize: 20, fontWeight: 400, color: C.white, margin: "0 0 6px" }}>Novo Negócio</h2>
        ...
        {quizStep === 2 && (
          <div>
            ...
            <button onClick={finishQuiz} style={{ width: "100%", padding: "14px", background: C.accent, color: C.bg, border: "none", borderRadius: 12, fontSize: 15, fontWeight: 700, fontFamily: FN, cursor: "pointer" }}>Criar negócio</button>
          </div>
        )}
      </div>
    );
    ...
    return (
      <div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <h2 style={{ fontFamily: TN, fontSize: 20, fontWeight: 400, color: C.white, margin: 0 }}>Meu Negócio</h2>
          <button onClick={() => setView("quiz")} style={{ padding: "8px 16px", borderRadius: 10, fontSize: 12, fontWeight: 600, fontFamily: MN, cursor: "pointer", background: `${C.accent}12`, color: C.accent, border: `1px solid ${C.accent}25` }}>+ Novo negócio</button>
        </div>
        ...
        {negocios.length === 0 && (
          <div style={{ textAlign: "center", padding: "40px 20px" }}>
            ...
            <button onClick={() => setView("quiz")} style={{ padding: "12px 28px", borderRadius: 12, fontSize: 14, fontWeight: 700, fontFamily: FN, cursor: "pointer", background: C.accent, color: C.bg, border: "none" }}>Adicionar negócio</button>
          </div>
        )}

        {showUpgrade && <UpgradeModal onClose={() => setShowUpgrade(false)} message="Desbloqueie o diagnóstico cruzado entre seus negócios e relatórios personalizados." />}
      </div>
    );
  }
  ...
  if (view === "historico" && active) {
    ...
    return (
      <div>
        ...
        <h2 style={{ fontFamily: TN, fontSize: 20, fontWeight: 400, color: C.white, margin: "0 0 6px" }}>Histórico — {active.nome}</h2>
  ...
  return (
    <div>
      {showUpgrade && <UpgradeModal onClose={() => setShowUpgrade(false)} message="Desbloqueie o relatório completo com diagnóstico personalizado para seu negócio." />}
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
          <div>
            <h2 style={{ fontFamily: TN, fontSize: 20, fontWeight: 400, color: C.white, margin: "0 0 2px" }}>{active.nome}</h2>
            <span style={{ fontSize: 11, color: C.accent, fontFamily: MN }}>{seg?.label}</span>
          </div>
          <div style={{ display: "flex", gap: 6 }}>
            <button onClick={() => setView("historico")}
              style={{ padding: "6px 14px", borderRadius: 8, fontSize: 10, fontFamily: MN, cursor: "pointer", background: `${C.accent}12`, color: C.accent, border: `1px solid ${C.accent}25` }}>Histórico</button>
      ...
      {active.meta > 0 && (
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "14px 18px", marginBottom: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
            ...
            <span style={{ fontFamily: MN, fontSize: 12, color: totalR >= active.meta ? C.accent : C.text }}>{fmtBRL(totalR)} / {fmtBRL(active.meta)}</span>
          </div>
      ...
      {/* DRE */}
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20, marginBottom: 16 }}>
        <div style={{ fontSize: 11, color: C.textMuted, fontFamily: MN, letterSpacing: "1px", marginBottom: 14 }}>RESULTADO DO MÊS</div>
        {[
          { label: "Receita Bruta", value: totalR, color: C.accent },
          { label: "(-) Custos e Despesas", value: totalD, color: C.red, neg: true },
          { label: "= Lucro Líquido", value: lucro, color: lucro >= 0 ? C.accent : C.red, bold: true },
        ].map((row) => (
          <div key={row.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: row.bold ? "none" : `1px solid ${C.border}`, borderTop: row.bold ? `1px solid ${C.border}` : "none", marginTop: row.bold ? 6 : 0 }}>
            <span style={{ fontSize: row.bold ? 14 : 13, color: row.bold ? C.white : C.textDim, fontWeight: row.bold ? 700 : 400 }}>{row.label}</span>
            <span style={{ fontFamily: MN, fontSize: row.bold ? 16 : 13, fontWeight: row.bold ? 800 : 600, color: row.color }}>{row.neg ? `- ${fmtBRL(row.value)}` : fmtBRL(Math.abs(row.value))}</span>
          </div>
        ))}
        {totalR > 0 && (
          <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
            <div style={{ flex: 1, padding: 10, background: C.cardAlt, borderRadius: 10, textAlign: "center", border: `1px solid ${C.border}` }}>
              <div style={{ fontSize: 9, color: C.textMuted, fontFamily: MN, marginBottom: 3 }}>MARGEM LÍQUIDA</div>
              <div style={{ fontFamily: MN, fontSize: 18, fontWeight: 800, color: margem >= bench.margemLiquida ? C.accent : C.yellow }}>{fmtPct(margem)}</div>
              <div style={{ fontSize: 9, color: C.textMuted, marginTop: 2 }}>Meta: {bench.margemLiquida}%</div>
            </div>
            <div style={{ flex: 1, padding: 10, background: C.cardAlt, borderRadius: 10, textAlign: "center", border: `1px solid ${C.border}` }}>
              <div style={{ fontSize: 9, color: C.textMuted, fontFamily: MN, marginBottom: 3 }}>CUSTOS / RECEITA</div>
              <div style={{ fontFamily: MN, fontSize: 18, fontWeight: 800, color: custoPct <= bench.custoTotal ? C.accent : C.yellow }}>{fmtPct(custoPct)}</div>
              <div style={{ fontSize: 9, color: C.textMuted, marginTop: 2 }}>Meta: até {bench.custoTotal}%</div>
            </div>
          </div>
        )}
      </div>
      ...
      {/* Alerts */}
      {alerts.map((a, i) => (
        <div key={i} style={{ padding: "12px 16px", marginBottom: 6, borderRadius: 10, background: a.type === "danger" ? `${C.red}08` : `${C.yellow}08`, border: `1px solid ${a.type === "danger" ? `${C.red}20` : `${C.yellow}20`}` }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: a.type === "danger" ? C.red : C.yellow, marginBottom: 2 }}>Atenção: {a.area}</div>
          {isPremium ? (
            <div style={{ fontSize: 11, color: C.textDim, lineHeight: 1.6 }}>{a.detail}</div>
          ) : (
            <div style={{ fontSize: 11, color: C.textDim }}>Ponto de melhoria identificado. <button onClick={() => setShowUpgrade(true)} style={{ background: "none", border: "none", color: C.accent, fontSize: 11, cursor: "pointer", fontFamily: FN, padding: 0, textDecoration: "underline" }}>Ver diagnóstico</button></div>
          )}
        </div>
      ))}
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20, marginBottom: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          ...
          <button onClick={() => { setShowModal("receitas"); setNewItem({ desc: "", valor: "", categoria: seg?.receitas[0] || "" }); }}
            style={{ padding: "5px 14px", borderRadius: 8, fontSize: 11, fontFamily: MN, cursor: "pointer", background: `${C.accent}12`, color: C.accent, border: `1px solid ${C.accent}25` }}>+ Adicionar</button>
        </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }} onClick={() => setShowModal(null)}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 400, background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: "24px 20px" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => addItem(showModal)} style={{ flex: 1, padding: 12, background: C.accent, color: C.bg, border: "none", borderRadius: 10, fontSize: 14, fontWeight: 700, fontFamily: FN, cursor: "pointer" }}>Salvar</button>
