"use client";import { useState, useMemo, useEffect } from "react";import { syncNegocios, carregarNegociosRemoto } from "../lib/supabaseSync";import { C, MN, FN, TN, heroStyle, moneyCompactStyle } from "../lib/theme";import SponsorSlot from "./SponsorSlot";import PremiumGate from "./PremiumGate";import PremiumNotice from "./PremiumNotice";import RelatorioPremium from "./meu-negocio/RelatorioPremium";import { BannerNegocio } from "./Banners";

const STORAGE_KEY = "comparai_negocios";

/* ═══════════════════════════════════════════════════════════════DATA═══════════════════════════════════════════════════════════════ */const SEGMENTOS = [{ id: "entregas", label: "Entregas / Motorista de App", desc: "iFood, Uber, 99, Rappi, entregas em geral", receitas: ["Corridas / Entregas", "Bônus e Incentivos", "Gorjetas", "Outras Receitas"], despesas: ["Combustível", "Manutenção do Veículo", "Seguro do Veículo", "Celular / Dados Móveis", "Alimentação na Rua", "Troca de Óleo", "Pneus", "Lavagem", "Outras Despesas"] },{ id: "alimentacao", label: "Alimentação / Salgados / Doces", desc: "Produção e venda de alimentos", receitas: ["Vendas Diretas", "Encomendas", "Eventos / Festas", "Revenda", "Outras Receitas"], despesas: ["Ingredientes / Insumos", "Embalagens", "Gás de Cozinha", "Energia Elétrica", "Água", "Aluguel do Espaço", "Equipamentos", "Transporte / Entrega", "Marketing", "Outras Despesas"] },{ id: "beleza", label: "Beleza / Estética / Barbearia", desc: "Cabelo, unhas, estética, maquiagem", receitas: ["Atendimentos", "Vendas de Produtos", "Pacotes Mensais", "Outras Receitas"], despesas: ["Produtos Profissionais", "Materiais Descartáveis", "Aluguel / Espaço", "Energia Elétrica", "Água", "Marketing", "Equipamentos", "Outras Despesas"] },{ id: "servicos_gerais", label: "Serviços Gerais / Manutenção", desc: "Eletricista, encanador, pintor, diarista", receitas: ["Prestação de Serviços", "Contratos Mensais", "Extras / Bicos", "Outras Receitas"], despesas: ["Ferramentas", "Materiais de Trabalho", "Transporte", "Celular / Dados", "EPI / Uniformes", "Outras Despesas"] },{ id: "comercio", label: "Comércio / Revenda / Loja", desc: "Venda de produtos físicos, loja, bazar", receitas: ["Vendas à Vista", "Vendas a Prazo", "Vendas Online", "Outras Receitas"], despesas: ["Custo da Mercadoria", "Frete / Envio", "Embalagens", "Aluguel", "Energia Elétrica", "Funcionários", "Marketing", "Taxas de Plataforma", "Outras Despesas"] },{ id: "digital", label: "Digital / Freelancer / Conteúdo", desc: "Designer, programador, social media, influencer", receitas: ["Projetos / Contratos", "Recorrência Mensal", "Vendas de Infoprodutos", "Publicidade / Parcerias", "Outras Receitas"], despesas: ["Ferramentas / Assinaturas", "Internet", "Equipamentos", "Marketing / Tráfego Pago", "Contador", "Outras Despesas"] },{ id: "outro", label: "Outro Segmento", desc: "Não se encaixa nas opções acima", receitas: ["Receita Principal", "Receita Secundária", "Outras Receitas"], despesas: ["Custo Principal", "Materiais", "Transporte", "Aluguel", "Marketing", "Outras Despesas"] },];

const FAIXAS = [{ id: "ate2k", label: "Até R$ 2.000/mês" },{ id: "2k_5k", label: "R$ 2.000 a R$ 5.000/mês" },{ id: "5k_10k", label: "R$ 5.000 a R$ 10.000/mês" },{ id: "10k_20k", label: "R$ 10.000 a R$ 20.000/mês" },{ id: "acima20k", label: "Acima de R$ 20.000/mês" },];
...
function calcTotals(neg) {const month = curMonth();const rec = (neg.receitas || []).filter((r) => (r.month || month) === month);const desp = (neg.despesas || []).filter((d) => (d.month || month) === month);const totalR = rec.reduce((s, r) => s + r.valor, 0);const totalD = desp.reduce((s, d) => s + d.valor, 0);return { totalR, totalD, lucro: totalR - totalD, margem: totalR > 0 ? ((totalR - totalD) / totalR) * 100 : 0, custoPct: totalR > 0 ? (totalD / totalR) * 100 : 0 };}

/* ═══════════════════════════════════════════════════════════════MAIN═══════════════════════════════════════════════════════════════ */export default function MeuNegocio({ user }) {const userId = user?.id || null;const [negocios, setNegocios] = useState([]);const [view, setView] = useState("list"); // list, quiz, dashboard, historicoconst [activeId, setActiveId] = useState(null);const [quizStep, setQuizStep] = useState(0);const [quizData, setQuizData] = useState({ segmento: null, faturamento: null, nome: "", meta: "" });const [showModal, setShowModal] = useState(null);const [newItem, setNewItem] = useState({ desc: "", valor: "", categoria: "" });const [showUpgrade, setShowUpgrade] = useState(false);const [editData, setEditData] = useState({ nome: "", meta: "", segmento: null, faturamento: null });const [loaded, setLoaded] = useState(false);const [viewMonth, setViewMonth] = useState(curMonth()); // for browsing other months

const isPremium = user?.is_premium || user?.is_admin;

useEffect(() => { setNegocios(loadAll()); setLoaded(true); }, []);// Carrega do Supabase na montagem se logadouseEffect(() => {if (!userId) return;carregarNegociosRemoto(userId).then((remoto) => {if (remoto && Array.isArray(remoto)) {try { localStorage.setItem(STORAGE_KEY, JSON.stringify(remoto)); } catch {}setNegocios(remoto);}});}, [userId]);

useEffect(() => { if (loaded) saveAll(negocios, userId); }, [negocios, loaded, userId]);

const active = negocios.find((n) => n.id === activeId);const seg = active ? SEGMENTOS.find((s) => s.id === active.segmento) : null;const bench = active ? (BENCHMARKS[active.segmento] || BENCHMARKS.outro) : BENCHMARKS.outro;

const month = curMonth();const displayMonth = viewMonth || month;const isCurrentMonth = displayMonth === month;const recMes = useMemo(() => (active?.receitas || []).filter((r) => (r.month || month) === displayMonth), [active, displayMonth, month]);const despMes = useMemo(() => (active?.despesas || []).filter((d) => (d.month || month) === displayMonth), [active, displayMonth, month]);const totalR = useMemo(() => recMes.reduce((s, r) => s + r.valor, 0), [recMes]);const totalD = useMemo(() => despMes.reduce((s, d) => s + d.valor, 0), [despMes]);const lucro = totalR - totalD;const margem = totalR > 0 ? (lucro / totalR) * 100 : 0;const custoPct = totalR > 0 ? (totalD / totalR) * 100 : 0;

const despPorCat = useMemo(() => {const g = {};despMes.forEach((d) => { g[d.categoria] = (g[d.categoria] || 0) + d.valor; });return Object.entries(g).sort((a, b) => b[1] - a[1]);}, [despMes]);

// (removido: useMemo de alerts antigo. O diagnóstico agora vive em// lib/meuNegocio/motor e é renderizado pelo componente RelatorioPremium.)

// === MUTATIONS ===const updateActive = (fn) => setNegocios((prev) => prev.map((n) => n.id === activeId ? fn(n) : n));const addItem = (type) => {if (!newItem.desc || !newItem.valor || !newItem.categoria) return;const item = { id: Date.now(), desc: newItem.desc, valor: parseFloat(newItem.valor), categoria: newItem.categoria, month: displayMonth };updateActive((n) => ({ ...n, [type]: [...(n[type] || []), item] }));setNewItem({ desc: "", valor: "", categoria: "" });setShowModal(null);};const removeItem = (type, id) => updateActive((n) => ({ ...n, [type]: (n[type] || []).filter((x) => x.id !== id) }));const deleteNegocio = (id) => { setNegocios((prev) => prev.filter((n) => n.id !== id)); setView("list"); setActiveId(null); };

const finishQuiz = () => {const novo = { id: Date.now(), segmento: quizData.segmento, faturamento: quizData.faturamento, nome: quizData.nome || SEGMENTOS.find((s) => s.id === quizData.segmento)?.label || "Meu Negócio", meta: parseFloat(quizData.meta) || 0, receitas: [], despesas: [], createdAt: new Date().toISOString() };setNegocios((prev) => [...prev, novo]);setActiveId(novo.id);setView("dashboard");setQuizData({ segmento: null, faturamento: null, nome: "", meta: "" });setQuizStep(0);};

/* ═══════════════════════════════════════════════════════════════QUIZ═══════════════════════════════════════════════════════════════ */if (view === "quiz") {return (<div style={{ marginBottom: 20 }}><button onClick={() => { setView("list"); setQuizStep(0); }} style={{ background: "none", border: "none", color: C.textDim, fontSize: 12, cursor: "pointer", fontFamily: FN, marginBottom: 12 }}>← VoltarNovo Negócio<p style={{ fontSize: 13, color: C.textDim, lineHeight: 1.7, margin: 0 }}>Configure o perfil para gestão personalizada.
...
/* ═══════════════════════════════════════════════════════════════LIST (HOME)═══════════════════════════════════════════════════════════════ */if (view === "list") {const consolidated = negocios.length >= 2 ? negocios.reduce((acc, n) => {const t = calcTotals(n);return { totalR: acc.totalR + t.totalR, totalD: acc.totalD + t.totalD, lucro: acc.lucro + t.lucro };}, { totalR: 0, totalD: 0, lucro: 0 }) : null;
...
/* ═══════════════════════════════════════════════════════════════HISTÓRICO═══════════════════════════════════════════════════════════════ */if (view === "historico" && active) {// Build monthly data from all receitas/despesasconst allMonths = new Set();(active.receitas || []).forEach((r) => allMonths.add(r.month || month));(active.despesas || []).forEach((d) => allMonths.add(d.month || month));const sortedMonths = [...allMonths].sort();
...
/* ═══════════════════════════════════════════════════════════════EDITAR NEGÓCIO (Premium)═══════════════════════════════════════════════════════════════ */if (view === "editar" && active && isPremium) {const seg = SEGMENTOS.find((s) => s.id === editData.segmento);return (<button onClick={() => setView("dashboard")} style={{ background: "none", border: "none", color: C.textDim, fontSize: 12, cursor: "pointer", fontFamily: FN, marginBottom: 12 }}>← Voltar ao dashboardEditar negócio<div style={{ background: C.card, border: 1px solid ${C.border}, borderRadius: 16, padding: 20, marginTop: 16, display: "grid", gap: 14 }}>
...
/* ═══════════════════════════════════════════════════════════════DASHBOARD═══════════════════════════════════════════════════════════════ */if (!active) { setView("list"); return null; }

return ({showUpgrade && <PremiumGate onClose={() => setShowUpgrade(false)} context="diagnostico" />}
