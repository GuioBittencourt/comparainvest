    ...
    <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 10 }}>
          {[
            ...
            <div key={item.label} style={{ background: C.cardAlt, border: `1px solid ${C.border}`, borderRadius: 14, padding: "12px 12px", minWidth: 0 }}>
              ...
              <div style={{ fontFamily: MN, fontSize: 15, fontWeight: 700, color: item.color, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{item.value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 18, padding: 20, marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 14 }}>
          <div>
            <div style={{ fontSize: 11, color: C.textMuted, fontFamily: MN, letterSpacing: "1px", marginBottom: 4 }}>RELATÓRIO DO MÊS</div>
            <div style={{ fontSize: 13, color: C.white, fontWeight: 650 }}>Diagnóstico financeiro do negócio</div>
          </div>
          {!isPremium && (
            <button onClick={() => setShowUpgrade(true)} style={{ padding: "7px 12px", borderRadius: 10, fontSize: 10, fontFamily: MN, cursor: "pointer", background: "rgba(255,255,255,0.030)", color: C.textDim, border: `1px solid ${C.border}` }}>
              Premium
            </button>
          )}
        </div>
        ...
        {isPremium ? (
          <div style={{ display: "grid", gap: 8 }}>
            <div style={{ padding: 12, borderRadius: 13, background: C.cardAlt, border: `1px solid ${C.border}` }}>
              <div style={{ fontSize: 11, color: C.textDim, lineHeight: 1.6 }}>
                {totalR > 0
                  ? `Sua margem líquida está em ${fmtPct(margem)}. A referência para ${seg?.label || "este segmento"} é cerca de ${bench.margemLiquida}%.`
                  : "Registre receitas e despesas para liberar o diagnóstico do mês."}
              </div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 8 }}>
              <div style={{ padding: 12, borderRadius: 13, background: C.cardAlt, border: `1px solid ${C.border}` }}>
                <div style={{ fontSize: 9, color: C.textMuted, fontFamily: MN, marginBottom: 4 }}>ECONOMIA POTENCIAL</div>
                <div style={{ fontFamily: MN, fontSize: 15, color: custoPct > bench.custoTotal ? C.yellow : C.accent, fontWeight: 700 }}>
                  {totalR > 0 && custoPct > bench.custoTotal ? fmtBRL(((custoPct - bench.custoTotal) / 100) * totalR) : "R$0,00"}
                </div>
              </div>
              <div style={{ padding: 12, borderRadius: 13, background: C.cardAlt, border: `1px solid ${C.border}` }}>
                <div style={{ fontSize: 9, color: C.textMuted, fontFamily: MN, marginBottom: 4 }}>PRIORIDADE</div>
                <div style={{ fontSize: 12, color: C.white, fontWeight: 650 }}>
                  {alerts[0]?.area || (totalR > 0 ? "Manter disciplina" : "Registrar dados")}
                </div>
              </div>
            </div>
          </div>
        ) : (
          <div style={{ padding: 14, borderRadius: 14, background: "rgba(255,255,255,0.025)", border: `1px solid ${C.border}` }}>
            <div style={{ fontSize: 12, color: C.textDim, lineHeight: 1.6 }}>
              O relatório premium cruza margem, custos, benchmark do segmento e economia potencial. Libere para transformar registros em diagnóstico.
            </div>
          </div>
        )}
      </div>
