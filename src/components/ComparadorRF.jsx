...
import { C, MN, FN, TN, PAL, premiumNoticeStyle } from "../lib/theme";
...
export default function ComparadorRF({ user, onSearch }) {
  ...
  const [upgradeContext, setUpgradeContext] = useState("rendaFixa");
  ...
  return (
    <div>
      {showUpgrade && (
        <PremiumGate onClose={() => setShowUpgrade(false)} context={upgradeContext} />
      )}
        ...
        {MACRO_SIGNALS.map((m, i) => (
          ...
          >
            <span style={{ fontSize: 14 }}>
              <span style={{ width: 10, height: 10, borderRadius: "50%", display: "inline-block", background: m.signal === "green" ? "#34D399" : m.signal === "yellow" ? "#FBBF24" : "#F87171" }} />
            </span>
            <div>
        ...
        >
          <div style={{ display: "flex", alignItems: "flex-start", gap: 10, minWidth: 0, flex: 1 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: C.white, lineHeight: 1.25 }}>Mais comparações, mais insights</div>
              <div style={{ fontSize: 11, color: C.textDim, lineHeight: 1.45, marginTop: 2 }}>Compare sem restrições e descubra padrões mais profundos.</div>
            </div>
          </div>
          <a href={PREMIUM_PAGE_URL} style={{ textDecoration: "none" }}>
            <button style={{ padding: "6px 12px", borderRadius: 8, fontSize: 10, fontWeight: 700, fontFamily: MN, cursor: "pointer", background: "rgba(16,185,129,0.055)", color: C.accent, border: `1px solid ${C.accent}30`, whiteSpace: "nowrap" }}>Ver benefícios</button>
          </a>
        </div>
      )}
