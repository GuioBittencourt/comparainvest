...
import { IconHome, IconCarteira, IconAcoes, IconProteger, IconNegocio } from "../components/Icons";
...
const navItems = [
  ...
  { id: "carteira", label: "Carteira", Icon: IconCarteira },
  { id: "comparadores", label: "Investir", Icon: IconAcoes },
  { id: "educacao", label: "Diagnóstico", Icon: IconProteger },
  { id: "meu-negocio", label: "Mais", Icon: IconNegocio },
];
      ...
      {tab !== "admin" && (
        ...
        >
          {navItems.map(({ id, label, Icon }) => {
            const active = tab === id || (id === "comparadores" && ["comparadores", "acoes", "fiis", "rf"].includes(tab));
            return (
              <button
                key={id}
                onClick={() => setTab(id)}
