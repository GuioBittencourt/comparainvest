...
export default function PremiumPage() {
  const [selecionado, setSelecionado] = useState("anual");
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // userId é a chave principal — webhook busca direto pelo id do Supabase
        if (session?.user?.id) setUserId(session.user.id);
      } catch {}
    })();
  }, []);

  // client_reference_id = userId do Supabase (busca direta no webhook)
  // prefilled_email = email pré-preenchido no checkout (UX + fallback)
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
              ...
              >
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: p.destaque ? C.white : C.text, marginBottom: 3 }}>
                      {p.nome}
                    </div>
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div style={{ fontFamily: MN, fontSize: 16, fontWeight: 700, color: p.destaque || ativo ? C.accent : C.text, lineHeight: 1.1 }}>
                      R$ {p.precoMes}
        ...
        <a href={buildStripeLink(planoAtual.id)} style={{ textDecoration: "none", display: "block", marginBottom: 10 }}>
        ...
        <a href="/" style={{ textDecoration: "none", display: "block" }}>
          <button style={{ ...btnSecondary, width: "100%" }}>Voltar</button>
        </a>

        <div style={{ marginTop: 24, padding: "16px 14px", background: "rgba(255,255,255,0.02)", border: `1px solid ${C.border}`, borderRadius: 12, textAlign: "center" }}>
          <div style={{ fontSize: 11, color: C.textDim, lineHeight: 1.7, display: "flex", flexDirection: "column", gap: 4 }}>
            <div><span style={{ color: C.accent, fontWeight: 600 }}>●</span> Pagamento seguro via Stripe</div>
            <div><span style={{ color: C.accent, fontWeight: 600 }}>●</span> Cancele quando quiser, sem multa</div>
            <div><span style={{ color: C.accent, fontWeight: 600 }}>●</span> Acesso liberado automaticamente após pagamento</div>
          </div>
        </div>

        <div style={{ fontSize: 10, color: C.textMuted, textAlign: "center", marginTop: 18, lineHeight: 1.6, fontStyle: "italic" }}>
