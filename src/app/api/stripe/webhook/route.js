 ...
 * /api/stripe/webhook
 *
 * Busca de perfil em cascata (3 tentativas):
 *   1. client_reference_id = user_id do Supabase (mais confiável)
 *   2. stripe_customer_id salvo no perfil
 *   3. email do Stripe vs email do Supabase (fallback)
 */
...
/* ══════════════════════════════════════════════════════════
   BUSCA DE PERFIL — 3 tentativas em cascata
   ══════════════════════════════════════════════════════════ */

async function encontrarProfile({ userId, customerId, email }) {
  // 1. Busca pelo user_id do Supabase (client_reference_id passado no checkout)
  if (userId) {
    const { data } = await supabase
      ...
      .maybeSingle();
    if (data) {
      console.log(`[webhook] perfil encontrado por userId: ${data.email}`);
      return data;
    }
  }

  // 2. Busca pelo stripe_customer_id já salvo no perfil
  if (customerId) {
    const { data } = await supabase
      .from("profiles")
      .select("id, email, is_premium")
      ...
      .maybeSingle();
    if (data) {
      console.log(`[webhook] perfil encontrado por customerId: ${data.email}`);
      return data;
    }
  }

  // 3. Fallback: busca pelo email (case insensitive)
  if (email) {
    const { data } = await supabase
      .from("profiles")
      .select("id, email, is_premium")
      ...
      .maybeSingle();
    if (data) {
      console.log(`[webhook] perfil encontrado por email: ${data.email}`);
      return data;
    }
  }
...
async function handleCheckoutCompleted(session) {
  ...
  // client_reference_id agora é o userId do Supabase
  ...
  const plano = detectarPlano(
    item?.price?.recurring?.interval,
    item?.price?.recurring?.interval_count
  );

  if (!plano) {
    console.error("[webhook] plano não identificado:", item?.price?.recurring);
    return;
  }
